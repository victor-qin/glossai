...
export const ANNOTATION_PROMPT = `You are a Japanese language expert. Given Japanese text, analyze it and return structured segments with readings.
...
- Segments must be in order so concatenating all "text" fields reproduces the full input.
- Use standard Hepburn romanization (shi not si, chi not ti, tsu not tu, fu not hu).
- For katakana words (loanwords), use a "kana" segment.
- Spaces between words in romaji should follow natural word boundaries.`;
